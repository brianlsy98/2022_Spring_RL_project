...
class agent():
    ...
    def __init__(self, e, head_num):
        ...
        for _ in range(self.ensemble_num):
            Qprin = DQN(e.observation_space.n, e.action_space.n, self.hidden_dim, optimizer = keras.optimizers.Adam(learning_rate=self.lr))
            Qtarg = DQN(e.observation_space.n, e.action_space.n, self.hidden_dim, optimizer = keras.optimizers.Adam(learning_rate=self.lr))
            self.Qs.append([Qprin, Qtarg])

        return
    

    def action(self, s):    # s : converted from integer to length 60 array
    ...
    def train(self):
        ...
        for iter in range(self.episode_length):
            ...
            self.state = np.zeros((self.env._shape[0]*self.env._shape[1],))
            self.state[state] += 1
            done = False
            rsum = 0
            ...
            obs_ = np.zeros((self.env._shape[0]*self.env._shape[1],))
            for pit in self.env._pits:
                obs_[pit] -= 100                                   # visualize pit as -100
            obs_[self.env.goal[0]*self.env._shape[1]+self.env.goal[1]] = 1000     # visuailze goal as 1000
            # ========================= #
            ...
            while not done:
                ...
                ###  ONE STEP  ###
                ##################
                c_state = self.state # array
                next_state, reward, done, _ = self.env.step(action)
                state = np.where(next_state == 1)[0]
                # next_state : array
                rsum += reward
                ##################
                ...
                # == only for visualizing == #
                print("")
                print(f"trajectory : at iter {iter}")
                print(obs_.reshape(self.env._shape))
                # ========================== #
                ...
                ### Update Buffer ###
                #####################
                buffer.append((c_state, action, reward, next_state, done, heads))
                #####################
