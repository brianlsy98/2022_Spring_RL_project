...
class agent():
    ...
    def train(self):
        ...
        for iter in range(self.episode_length):
            ...
            if iter % 10 == 0:
                m = np.mean(r_record[-10:])
                print('iteration {} ave reward {}'.format(iter, m))
                if m > 8.5 : good_means.append(m)
                if len(good_means) > 5 : print("converged after : "+str(iter)+" steps"); break
